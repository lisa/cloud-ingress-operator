...
type CloudClient interface {
	...
	/* SSH */
	// EnsureSSHDNS ensures there's a rh-api (for example) alias to the Service for the SSH pod
	EnsureSSHDNS(context.Context, client.Client, *cloudingressv1alpha1.SSHD, *corev1.Service) error

	// DeleteSSHDNS will ensure that the A record for the SSH pod (rh-api) is removed
